...
def seq_aug(seq):
    for i in range(len(seq)):
        seq[i] = seq[i][6:]+'+'+seq[i][:5]
    return seq

df_mixed_penta = pd.read_csv('mixed_penta_demo.csv')
test_feat = np.array(df_mixed_penta["Feature"])

predict = []

test_enc_inputs = make_data(test_feat,src_len).to(device)
model_load = Transformer()
checkpoint = torch.load('model_mixed_penta.pt')
...
df_mixed_penta['predict'] = predict
df_mixed_penta.to_csv('AP_mixed_penta.csv')
